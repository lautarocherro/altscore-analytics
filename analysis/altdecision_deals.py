...
COLORS = {"Low Ticket": "#1f77b4", "High Ticket": "#ff7f0e"}
...
def main():
    ...
    st.markdown("## 💼 AltDecision · Deal Amount Analysis")
    st.caption("New Business deals that entered demo from **October 2025 to December 2026**")
    ...
    c1.metric("Total Deals", f"{len(df):,}")
    c2.metric("Mean Amount", f"${df['amount'].mean():,.0f}")
    c3.metric("Median Amount", f"${df['amount'].median():,.0f}")
    c4.metric("Max Amount", f"${df['amount'].max():,.0f}")
    c5.metric("Cluster Boundary", f"${boundary_usd:,.0f}")
